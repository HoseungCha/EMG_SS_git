function draw_EMG_over_channel(handles)
%DRAW_ALPHABET_SET Summary of this function goes here
%   Detailed explanation goes here

% choose specific channel to plot
prompt = {'Will you save figure as jpg?'};
dlg_title = 'Set saving';
num_lines = 1;
def = {'no'};
...
if isfield(buffer, 'trial_data') && ~isempty(buffer.trial_data{1,1}.data_queue)
    ...
    [N_words, ~] = size(Data);
    [~, N_ch]=  size(Data{1}.data_queue);
    N_cols = ceil(N_words);
    
    %% Data Retrieve
    max_values=zeros(N_ch,1);
    min_values=zeros(N_ch,1);
    ...
    for j = 1:N_words
        for i=1:N_ch
            if isempty(Data{j, i }.data_queue)
                ...
                data_arrangements_for_plot{i,j} = Data{j, 1 }.data_queue(:,i);
                titles_arrangements_for_plot{i,j}=Data{j,1}.name;
            end
    ...
    if strcmpi('Yes',fanswer{1,1})
        ...
        num_rows=(floor(0.5/awidth));
        num2plot_on_s=ceil(N_ch/num_rows);
        for i=1:num2plot_on_s
            if (num_rows*i>N_ch)
                fig=subplot_fullsize(N_cols,(N_ch-num_rows*(i-1)),...
                    data_arrangements_for_plot(num_rows*(i-1)+1:N_ch,:),y_range,titles_arrangements_for_plot);
            else
    ...
    if ~(strcmpi('Yes',fanswer{2,1}))
        [~,~,ax,axis_pos]=scrollfigdemo(N_words,N_ch, data_arrangements_for_plot,y_range,titles_arrangements_for_plot);
    end
    %     ax =reshape(ax,[N_ch,N_words]);
    %     axis_pos=reshape(axis_pos,[N_ch,N_words]);
