...
for i_ch_config = 1 : n_ch_config
% memory alloation
features = zeros(n_seg,n_feat2use{i_ch_config},n_word,n_trl,n_sub);
for i_sub = 1 : n_sub
    %subject name
    ...
    for i_trl = 1 : n_trl
        ...
        for i_win = 1 : n_win
            ...
            % TEAGER
            temp_teager = zeros(1,n_bp{i_ch_config});
            for i = 1 : n_bp{i_ch_config}
                tmp = cal_freqweighted_energy(curr_win(:,i),1,'teager');
                temp_teager(i) = sum(tmp)/length(tmp);
            end
            % HJORTH PARAMETERS
            temp_Hjorth = zeros(1,n_bp{i_ch_config}*3);
            for i = 1 : n_bp{i_ch_config}
        ...
        clear temp_rms temp_CC temp_WL temp_SampEN temp_min_max temp_teager ...
            temp_Hjorth
        %-----------------------------------------------------------------%
...
%                         idx_trg_start(i_emo_orer_in_this_exp)+floor((period_FE*fp.SF2use)/n_wininc)-1 ,:)];
        ...
        for i_feat_type = 1 : n_ftype
            ...
            subplot(n_ftype,1,i_feat_type);
            tmp = temp_feat(1:end,idx_feat2use{i_ch_config}{i_feat_type});
            plot(tmp);
            hold on;
            ylim([min(min(tmp(100:end,:))),max(max(tmp(100:end,:)))]);
            tmp = get(gca,'YLim');
            stem(idx_trg_start,repmat(tmp(1),[n_word,1]),'r','LineWidth',2);
            stem(idx_trg_start,repmat(tmp(2),[n_word,1]),'r','LineWidth',2);
            drawnow;
        end
        tightfig
        % plot the DB of subject
        savefig(gcf,fullfile(path_save,sprintf('subject-%s trial-%d',...
            tmp_name_sub(1:3), i_trl)));
        close
        %-----------------------------------------------------------------%
    end  
